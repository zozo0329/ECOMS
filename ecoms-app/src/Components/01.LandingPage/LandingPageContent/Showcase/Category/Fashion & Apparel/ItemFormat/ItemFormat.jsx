import React from "react";

const ItemFormat = ({ name, price, description, image, alt }) => {
  return (
    <div>
      <div className="aspect-3/4 overflow-hidden">
        <img
          src={image}
          alt={alt}
          className="w-full h-full object-cover hover:scale-105 transition-transform duration-300 cursor-pointer"
        />
      </div>
      <div className="p-4">
        <h3 className="font-medium text-gray-900 truncate cursor-default">
          {name}
        </h3>
        <p className="text-gray-500 text-sm mt-1 line-clamp-2 cursor-default">
          {description}
        </p>
        <div className="flex items-center justify-between mt-3">
          <span className="text-lg font-semibold text-gray-900 cursor-default">
            ${price}
          </span>
          <button className="px-2 py-2 bg-gray-900 text-white font-semibold rounded-lg hover:bg-amber-500 transition-colors duration-300 cursor-pointer">
            Add to Cart
          </button>
        </div>
      </div>
    </div>
  );
};

export default ItemFormat;
